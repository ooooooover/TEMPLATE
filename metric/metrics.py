...
def DL_score(feature,Trend_feature):
    if feature.shape[0]>feature.shape[1]:
        XT_X = np.dot(feature.T, feature)
    else:
        XT_X = np.dot(feature, feature.T)
    eigenvalue, x = power_iteration(XT_X)
    
    if Trend_feature.shape[0]>Trend_feature.shape[1]:
        XT_X = np.dot(Trend_feature.T, Trend_feature)
    else:
        XT_X = np.dot(Trend_feature, Trend_feature.T)
    eigenvalue, x_trend = power_iteration(XT_X)
    dl_score, _ = pearsonr(x, x_trend)
    return dl_score

def PL_score(feature):
    ...
    eigenvalues, _ = np.linalg.eigh(XT_X)
    ...
    pl_score = max_singular_value / nuclear_norm if nuclear_norm > 0 else 0

    return pl_score
    
from CKA import CKA, CudaCKA
import torch
def TA_score(feature, first_feature):
    
    device = torch.device('cuda')
    cuda_cka = CudaCKA(device)
    feature = torch.tensor(feature).to(device) 
    first_feature = torch.tensor(first_feature).to(device)
    
    ta_score = cuda_cka.kernel_CKA(first_feature, feature,sigma=None).cpu().numpy()
    
    return ta_score

def TEMPLATE(feature, first_feature,trend_feature):
    
    dl_score = DL_score(feature,trend_feature)
    pl_score = PL_score(feature)
    ta_score = TA_score(feature, first_feature)
    
    return dl_score,pl_score,ta_score
